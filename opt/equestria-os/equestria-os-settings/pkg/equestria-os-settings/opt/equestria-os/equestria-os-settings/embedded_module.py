...
class EmbeddedAppModule(BaseModule):
    ...
    def build_widget(self) -> QWidget:
        ...
        # Wrap the QMainWindow in a plain QWidget container.
        # Problem: apps call resize(W, H) in setupUi(), which makes Qt report
        # a large minimumSizeHint() for the QMainWindow.  QSplitter uses
        # minimumSizeHint() to calculate the minimum size of each pane — if
        # the embedded window reports e.g. 1100 px and the total window is
        # 1200 px, there is no room left for the sidebar and the splitter
        # becomes completely immovable.
        # Solution: wrap _win in a plain QWidget whose minimumSizeHint() is
        # always small (Qt default ≈ 0).  The inner _win uses Ignored policy
        # so it fills the container without propagating its own size hint.
    ...
    def _inject_checkmark_icons(self) -> None:
        """
        Apply custom SVG checkmark icons to every QCheckBox in the embedded app.

        Strategy: append override rules to the *window* stylesheet itself.
        Within a single stylesheet "last rule wins" for equal specificity — this
        is a Qt guarantee.  Appending ID-qualified selectors
        (``QCheckBox#Name::indicator:checked``) at the very end of the window
        stylesheet therefore reliably overrides any earlier rule with the same
        selector that used ``image: url(none)`` or a colored background.

        Widget-level setStyleSheet() is also called as a belt-and-suspenders
        measure, but the window-append is the primary mechanism.
        """
        ...
        for cb in checkboxes:
            obj = cb.objectName()
            # Prefer objectName-qualified selector (same specificity as any
            # existing ID-qualified rule in the window stylesheet → last wins).
            ...
            # Belt-and-suspenders: also set directly on the widget
            cb.setStyleSheet(cb.styleSheet() + win_append)

        # Append to the window stylesheet AFTER all existing rules so
        # same-specificity rules here win via "last rule wins" ordering.
